context("ruin_probability")

skip_on_cran()

#-------------------------------------------------------------------------------
# CramerLundberg model

# exact ruin probabilities from paper [1]

exact_probabilities <- data.frame(
  time_horizon = c(rep(100, 10),
                   rep(1000, 15),
                   rep(10000, 20)),
  initial_capital = c(rep(0, 5), rep(100, 5),
...
for(i in 1:nrow(exact_probabilities)) {
  ...
  #          ", premium_rate: ", exact_probabilities[i, "premium_rate"],
  #          ", time_horizon: ", exact_probabilities[i, "time_horizon"],
  #          ". A simulated lower bound of probability is less than exact one."),
  #   expect_true(
  #     rp$ruin_probability["lower_bound"] < exact_probabilities[i, "probability"]
  #   )
  # )
  #
  # test_that(
  ...
  #          ", premium_rate: ", exact_probabilities[i, "premium_rate"],
  #          ", time_horizon: ", exact_probabilities[i, "time_horizon"],
  #          ". A simulated upper bound of probability is grater than exact one."),
  #   expect_true(
  #     rp$ruin_probability["upper_bound"] > exact_probabilities[i, "probability"]
  #   )
  # )
...
#   probability = c(0.999999, 0.999962, 0.858597, 0.822716, 0.785428,
...
#
# #-------------------------------------------------------------------------------
# # CramerLundbergCapitalInjections model
#
# # exact ruin probabilities from paper [2]
#
# exact_probabilities <- data.frame(
